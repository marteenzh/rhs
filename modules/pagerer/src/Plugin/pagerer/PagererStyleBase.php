...
abstract class PagererStyleBase extends PluginBase implements PagererStyleInterface, PluginFormInterface, ContainerFactoryPluginInterface {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $config['display_container'] = array(
      '#type' => 'details',
      '#title' => $this->t("Main options"),
    );
    if (isset($d['quantity'])) {
      $config['display_container']['quantity'] = array(
        '#type' => 'number',
    ...
    if (isset($d['display_mode'])) {
      ...
      $config['display_container']['display_mode'] = array(
        '#type' => 'radios',
    ...
    if (isset($d['prefix_display'])) {
      $config['display_container']['prefix_display'] = array(
        '#type' => 'checkbox',
    ...
    if (isset($d['suffix_display'])) {
      $config['display_container']['suffix_display'] = array(
        '#type' => 'checkbox',
    ...
    if (isset($d['display_restriction'])) {
      ...
      $config['display_container']['display_restriction'] = array(
        '#type' => 'radios',
    ...
    if (isset($d['progr_links'])) {
      $options = array('relative', 'absolute');
      $options = array_combine($options, $options);
      $config['display_container']['progr_links'] = array(
        '#type' => 'select',
    ...
    // Links configuration.
    $config['links_container'] = array(
      '#type' => 'details',
      '#title' => $this->t("Links"),
      '#description' => $this->t("Configure link elements like '« First', '‹ Previous', 'Next ›' and 'Last »'."),
    );
    if (isset($d['first_link'])) {
      $options = array('never', 'not_on_first', 'always');
      $options = array_combine($options, $options);
      $config['links_container']['first_link'] = array(
        '#type' => 'select',
        '#title' => $this->t("First"),
        '#options' => $options,
    ...
    if (isset($d['previous_link'])) {
      $options = array('never', 'not_on_first', 'always');
      $options = array_combine($options, $options);
      $config['links_container']['previous_link'] = array(
        '#type' => 'select',
    ...
    if (isset($d['next_link'])) {
      $options = array('never', 'not_on_last', 'always');
      $options = array_combine($options, $options);
      $config['links_container']['next_link'] = array(
        '#type' => 'select',
        '#title' => $this->t("Next"),
        '#options' => $options,
    ...
    if (isset($d['last_link'])) {
      $options = array('never', 'not_on_last', 'always');
      $options = array_combine($options, $options);
      $config['links_container']['last_link'] = array(
        '#type' => 'select',
        '#title' => $this->t("Last"),
        '#options' => $options,
    ...
    if (isset($d['tags']['page_breaker'])) {
      $config['separators_container']['page_breaker'] = array(
        '#type' => 'textfield',
        ...
        '#states' => array(
          'visible' => array(
            ':input[name="breaker_display"]' => array('checked' => TRUE),
          ),
        ),
      );
    }
    if (isset($d['separator_display'])) {
      $config['separators_container']['separator_display'] = array(
        '#type' => 'checkbox',
    ...
    if (isset($d['tags']['page_separator'])) {
      $config['separators_container']['page_separator'] = array(
        '#type' => 'textfield',
        ...
        '#states' => array(
          'visible' => array(
            ':input[name="separator_display"]' => array('checked' => TRUE),
          ),
        ),
      );
    }
    ...
    foreach ($d['tags'] as $tags_key => $tags_set) {
      if (in_array($tags_key, array('pages', 'items', 'item_ranges'))) {
    ...
    foreach ($default_configuration as $conf_key => $conf_item) {
      ...
      elseif ($conf_key == 'tags') {
        foreach ($conf_item as $tag_key => $tag_value) {
          ...
          elseif (in_array($tag_key, array('pages', 'items', 'item_ranges'))) {
            ...
            foreach ($tag_value as $tag_display_key => $tag_display_value) {
              $tag_display_value = $form_state->getValue(['tags_container', $tag_key, $tag_display_key]);
